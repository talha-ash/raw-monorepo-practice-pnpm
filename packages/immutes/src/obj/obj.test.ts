...
describe('newObj', () => {
  ...
  it('should be strict equal given initial', () => {
    const obj = Obj.newObj({ a: 10, b: 20 })
...
describe('merge', () => {
  ...
  it('check ref differentiation', () => {
    ...
    const newUser = Obj.merge(user, address)
    const updatedNewUser = Obj.updateByPath(
      newUser,
      ['address', 'street', 'name'],
      'cloud',
    )
...
describe('deleteKey', () => {
  ...
  it('ignore delete invalid key from obj ', () => {
    const user = Obj.newObj({ id: 10, name: 'john' })
    expect(Obj.deleteKey(user, 'name' as any)).toEqual({ id: 10 })
  })
})
describe('deleteKeyByPath', () => {
  it('delete nested key from obj', () => {
    ...
    const address = { address: { street: 10 } }
    const newUser = Obj.merge(user, address)
    const afterDeleteStreetKey = Obj.deleteKeyByPath(newUser, [
      'address',
      'street',
    ])
